...
/**
 * Returns the full odontogram state as a plain object (for saving to backend).
 */
export function getOdontogramState(): object {
  ...
  return {
    version: "1.3",
    globals: { wisdomVisible, showBase, occlusalVisible, showHealthyPulp, edentulous },
    teeth,
  };
}

/**
 * Loads odontogram state from a plain object (for restoring from backend).
 */
export function loadOdontogramState(data: unknown): void {
  importStatus(data);
}
